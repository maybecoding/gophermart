...
func (uc *AuthUseCase) Register(ctx context.Context, login entity.UserLogin, pwd entity.UserPassword) (entity.Token, error) {
	...
	if err != nil {
		return "", fmt.Errorf("AuthUseCase - Register - order.LoginAvailable: %w", err)
	}
	if !loginAvail {
	...
	if err != nil {
		return "", fmt.Errorf("AuthUseCase - Register - order.UserNew: %w", err)
	}
	if user == nil {
...
func (uc *AuthUseCase) Login(ctx context.Context, login entity.UserLogin, pwd entity.UserPassword) (entity.Token, error) {
	...
	if err != nil {
		...
		return "", fmt.Errorf("AuthUseCase - Login - order.UserFind: %w", err)
	}
	if user == nil {
		return "", entity.ErrUserNotFound
	}
